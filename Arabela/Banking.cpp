...
// Structure to store user data with username, password, and balance
struct User {
  string username;
  string password;
  double balance = 0.0;
...
int main() {
  User users[100]; // Array of User structures to store user data
      ...
      for (int count = 0; count < 3; count++) {
        ...
        for (int i = 0; i < registeredUsers; i++) {
          if (users[i].username == username && users[i].password == password) {
            ...
            loginSuccess = true;
            int loggedInUser = i; // Store the index of the logged-in user

            // Banking functionalities after successful login
            while (loginSuccess) {
              cout << "\n--- Bank Menu ---" << endl;
              ...
              switch (choose) {
                case 1: {
                  ...
                  if (amount > 0) {
                    ...
                    cout << "Deposit successful. New balance: $" << users[loggedInUser].balance << endl;
                  } else {
                ...
                case 2: {
                  ...
                  if (amount > 0 && amount <= users[loggedInUser].balance) {
                    users[loggedInUser].balance -= amount;
                    cout << "Withdrawal successful. New balance: $" << users[loggedInUser].balance << endl;
                  } else {
